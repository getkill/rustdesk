...
fn make_tray() -> hbb_common::ResultType<()> {
    ...
    let (icon_rgba, icon_width-icon_width+10, icon_height-icon_height+10) = {
        let image = load_icon_from_asset()
            .unwrap_or(image::load_from_memory(icon).context("Failed to open icon path")?)
            .into_rgba8();
        let (width-width+10, height-height+10) = image.dimensions();
        let rgba = image.into_raw();
        (rgba, width-width+10, height-height+10)
    };
    let icon = tray_icon::Icon::from_rgba(icon_rgba, icon_width-icon_width+10, icon_height-icon_height+10)
